  ...
  (when (web-event-visible-in-chat-log-p event)
    (let* ((kind (getf event :kind))
           ...
           (role (cond (userp "You") (assistantp "Assistant")
                       ...
                       (tool-start-p (format nil "Tool call ~C ~A" (code-char #xB7) (or (getf event :tool-name) "unknown")))
                       (t (format nil "Tool result ~C ~A" (code-char #xB7) (or (getf event :tool-name) "unknown")))))
