...
(defun run-claude-backend-tests ()
  ...
  (flet ((fixture-response (name)
    ...
    (let ((response (claude-parse-stream-response
                     (format nil "{\"type\":\"rate_limit_event\",\"rate_limit_info\":{}}~%{\"notice\":\"future-record-without-type\"}~%{\"type\":\"result\",\"subtype\":\"success\",\"result\":\"unknowns-safe\",\"session_id\":\"fixture\"}")
                     (claude-test-request))))
      (ensure-equal "unknowns-safe" (completion-response-text response)
                    "unknown and typeless stream records do not break final-result parsing"))
    (let ((response (fixture-response "web-pseudotool-text")))
      (ensure-equal '() (completion-response-native-tool-events response)
                    "live web pseudo-tool markdown must not fabricate tool cards")
      (ensure-true (search "mcp__harness__run_shell" (completion-response-text response))
                   "pseudo-tool fixture faithfully retains the observed final text"))
    (let* ((response (fixture-response "browser-domain-failure"))
           (event (first (completion-response-native-tool-events response))))
      (ensure-equal "browser_click" (getf event :tool-name)
                    "domain-failure fixture retains normalized non-shell tool name")
      (ensure-true (search "Browser is not open" (getf event :result))
                   "domain failure content is preserved even when protocol is_error is absent"))
